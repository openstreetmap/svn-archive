	...
	if(inDoc)
	{
		if(element=="node")
		{
			...
			for(int count=0; count<attributes.length(); count++)
			{
				if(attributes.qName(count)=="lat")
					lat = atof(attributes.value(count).toAscii().constData());
				else if(attributes.qName(count)=="lon")
					lon = atof(attributes.value(count).toAscii().constData());
				else if(attributes.qName(count)=="id")
					curID = atoi(attributes.value(count).toAscii().constData());
			}
		...
		else if(element=="segment")
		{
			...
			for(int count=0; count<attributes.length(); count++)
			{
				...
				{
					from = atoi(attributes.value(count).toAscii().constData());
				}
				else if(attributes.qName(count)=="to")
				{
					to = atof(attributes.value(count).toAscii().constData());
				}
				else if(attributes.qName(count)=="id")
				{
					curID  = atoi(attributes.value(count).toAscii().constData());
		...
		else if (element=="seg" && (inWay||inArea))
		{
			...
			for(int count=0; count<attributes.length(); count++)
			{
				if(attributes.qName(count)=="id")
				{
					segID=atoi(attributes.value(count).toAscii().constData());
				}
			}

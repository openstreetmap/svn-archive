		...
		function selectUp(f)
		{
			...
			else if (f instanceof OpenLayers.OSMItem)
			{
				alert('XML: ' + f.tagsToXML());
			}
			...
			if(f.type=="segment")
			{
				this.selectedSegments[this.selectedSegments.length] = f.osmid;
			}
			*/
		}
			...
			if(selectedFeature)
			{
					...
					OpenLayers.Geometry.Polygon && p ))
				{
					...
					if(newTags)
					{
						...
						var URL = 
						'http://www.free-map.org.uk/common/osmproxy2.php'
        ...
        function init(){
			...
			*/
			...
			var blank = new OpenLayers.Layer.WMS( "None", 
				"http://www.free-map.org.uk/common/trackpoints.php",
				{buffer:1} );

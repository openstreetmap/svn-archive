dojo.provide("cmyk.css.CSSParser");

/*

Copyright (C) Trevor McCauley
http://www.senocular.com/index.php?id=1.289

Modified by Mario Ferraro for CMYK library
...
//if used \n| then everything before the last comment is trimmed!
...
/*	if (styles.length){
	...
	// Workaround for 4Kb bound limits, thanks to http://www.howtocreate.co.uk/tutorials/javascript/dombasics
	if (styles.length){
		var i, len = styles.length;
		for (i=0; i<len; i++) {
			var i_children, len_children = styles[i].childNodes.length;
			for (i_children=0; i_children<len_children; i_children++) {
			//Workaround for comments node, they're stripped with nodeValue!
			if (styles[i].childNodes[i_children].nodeType==Node.TEXT_NODE) {
					style_str += styles[i].childNodes[i_children].nodeValue;
				}
			}
		}
	}
...
CSSParser.prototype.getStyles = function() {
	return this.styles;
}

CSSParser.prototype.getWriterString = function(style) {
this.styles = style;
	var backToString = "";
	for (object in this.styles) {
		backToString += "."+this.styles[object].selectors[0].singleSelectors[0].classes.values+"{\r\n";
		for (properties in this.styles[object].selectors[0].properties.values) {
			if (typeof(properties) != "function") {
				backToString += "\t"+properties+" : ";
				backToString += this.styles[object].selectors[0].properties.values[properties]+";\r\n";
			}
		}
		backToString += "}\r\n\r\n";
	}

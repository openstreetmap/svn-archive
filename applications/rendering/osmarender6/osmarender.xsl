...
<xsl:stylesheet 
  ...
  version="1.0">
	...
	<xsl:template name='generateAreaSubPath'>
		...
		<xsl:message>WayId: <xsl:value-of select='$way/@id'/> Loop: <xsl:value-of select='$loop'/> Loop: <xsl:value-of select='$way/nd[1]/@ref'/> Loop: <xsl:value-of select='$way/nd[last()]/@ref'/></xsl:message>
	...
	<!-- Generate an area path for the current way or area element -->
    <xsl:template name="xgenerateAreaPath">

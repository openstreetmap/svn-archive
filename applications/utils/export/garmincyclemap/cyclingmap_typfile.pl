#!/usr/bin/perl -w
	...
	# Preprocess .osm file for cycling map
	...
	# Richard Fairhurst 8-13.1.08, 6.4.08, extremecarver, 06.10.08
...
# =====	Constants
	...
	%usetag=('name',1, 'ref',1, 'route',1,
			 ...
			 'rcn',1, 'rcn_ref',1,
			 'lcn',1, 'lcn_ref',1);
	open (INFILE,"resources/cycling_map_features_typfile.csv") or die "Can't find cycling_map_features_typfile: $!\n";
	while (<INFILE>) {
		chomp ($t=$_);
		if ($t=~/^\w+\|(\w+)/) { $usetag{$1}=1; }
	}
...
#			if ($cycle!=NONE or $cycleref) { print "$refnum ($highway)\n"; }
	...
	# ===== Call osmcut to split into smaller files (undo uncomment if you need osmcut)
	#		(probably save in temporary directory)
	# $tempdir='temp';
	# print "Splitting with osmcut\n";
	# system ("mkdir $tempdir");
# system ("./osmcut --force -s 2 -d $tempdir $ofn");
# system ("java -Xmx1256M -jar osmcut.jar 0.5 $ofn $tempdir");
#	# system ("del $ofn");
	...
	# changed to have some more zoomlevels, and more processing power as I commented out the osmcut which produced mistakes for my map. Change description (optional) if working with osm files other than Austria. I also commented out the delete process as instead of producing a gmapsupp.img I prefer inserting the tiles into mapsource.
#	system ("java -Xmx1256M -jar mkgmap.jar --description=Osm_Austria_v2 --map-features=resources/cycling_map_features_typfile.csv --levels=0:24,1:22,2:20,3:18,4:16,5:14,6:12,7:10,8:8 --net $tempdir/*");
	system ("java -Xmx1256M -jar mkgmap.jar --description=Osm_Austria_v2 --map-features=resources/cycling_map_features_typfile.csv --levels=0:24,1:22,2:20,3:17,4:14,5:12,6:9 --net *.osm.processed");
	print "Deleting temporary files\n";
	system ("del *.processed");
	# system ("del 6*.img");
	# system ("del 6*.tdb");
	#system ("rmdir /S/Q $tempdir");
	#move maps to maps folder
		# system ("mkdir $mapdir");
		# system ("move *.img \$mapdir");
	print "Finished!\n";
	system ("pause");

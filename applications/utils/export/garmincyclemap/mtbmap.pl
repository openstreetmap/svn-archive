#!/usr/bin/perl -w
	...
	# - put mtb-map-features.csv in mkgmap/resources
	...
	%usetag=('name',1, 'ref',1, 'route',1,
			 ...
			 'rcn',1, 'rcn_ref',1,
			 'lcn',1, 'lcn_ref',1);
	open (INFILE,"resources/mtb-map-features.csv") or die "Can't find mtb styles: $!\n";
	while (<INFILE>) {
		chomp ($t=$_);
		if ($t=~/^\w+\|(\w+)/) { $usetag{$1}=1; }
	}
...
#
#			#	munge highway tag for mtb tags
#			if (exists $roads{$highway}) { $hwp=$roads{$highway}; }
#									else { $hwp='offroad'; }		
#			if ($cycle==mtb_trail_type_singletrail) { $highway="singletrail_$hwp"; }
#		  elsif ($cycle==mtb_trail_type_downhill) { $highway="downhill_$hwp"; }
#		  elsif ($cycle==mtb_trail_type_freeride) { $highway="freeride_$hwp"; }
#		  elsif ($cycle==mtb_designated         ) { $highway="mtb_$hwp"; }
#			elsif ($cycle==mtb_yes                ) { $highway="mtb_$hwp"; }
#			if ($highway) { $tags{'highway'}=$highway; }
...
#			if ($cycle!=NONE or $cycleref) { print "$refnum ($highway)\n"; }
	...
	# system ("java -Xmx1256M -jar osmcut.0.5.jar 1 $ofn $tempdir");
	# system ("del $ofn");
	...
	system ("java -Xmx1256M -jar mkgmap.jar --description=Osm_Austria_v2 --map-features=resources/mtb-map-features.csv --levels=0:24,1:22,2:20,3:17,4:14,5:12,6:9 --net *.osm.processed");

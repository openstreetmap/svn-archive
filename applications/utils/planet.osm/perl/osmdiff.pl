...
# size given is the size of longitude in pixels
#
...
#constants
##########
my $version = "V1.1 beta (001)" ;
my $usage   = "osmdiff.pl file1.osm file2.osm output.htm pic.png [size [thickness]]" ;
...
sub draw_pic {
	...
	#my $new = $image->colorAllocate(0, 255, 0) ; #green
	my $new = $image->colorAllocate(0, 0, 0) ; #black
	my $changed = $image->colorAllocate(0, 0, 255) ;
	#my $changedtags = $image->colorAllocate(0, 255, 0) ; # green light
		...
		if ( $#{$way0_nodes{$key}} > 0 ) {
			for ($i=0 ; $i < $#{$way0_nodes{$key}}; $i++) {
				if (      (defined($node0_lon{$way0_nodes{$key}[$i]})) and (defined($node0_lon{ $way0_nodes{$key}[$i+1]}))   ) {
					...
					#print $x1, " ", $y1, " ", $x2, " ", $y2, "\n" ;
					$image->line($x1,$y1,$x2,$y2,$background) ;
				}
				else
				{

...
my %Config;
  ...
  else
  {
    ...
    $Config{DownLoadURL}= 'http://www.freemap.sk';
...
sub DownloadTile()
{
  ...
  my $ImageFile;
      ...
      {
          # stiahnut tile
          $URL = sprintf("%s/%s/%d/%d/%d.png",$Config{DownLoadURL}, $layer,$Zoom, $X, $Y);
          $ImageFile= sprintf("%d_%d_%d.png", $Zoom, $X, $Y);
          ...
          if (-e $ImageFile)
          {
             if (-s $ImageFile >128)
             {
                my $SubImage = newFromPng GD::Image($ImageFile);

...
void SRTMDataGrid::doLoad(LATLON_TILE *rect,Map& map,int index)
{
...
void SRTMDataGrid::getLine(LINE *lines,int *n_lines, int ht)
{
	int go = 0;
	int prevedges[2];
	LINE line;
	double eAh0, eAh1, eBh0, eBh1; 
	int eAp0x, eAp0y, eAp1x, eAp1y, eBp0x, eBp0y, eBp1x, eBp1y;
		...
		if(between(ht,points[edges[edge][0]].hgt,points[edges[edge][1]].hgt))
		{
			for(int edge2=edge+1; edge2<4; edge2++)
			{
						 ...
						 points[edges[edge2][1]].hgt))
				{
					...
					eAp0x = points[edges[edge][0]].screenPos.x;
					eAp0y = points[edges[edge][0]].screenPos.y;
					eAp1x = points[edges[edge][1]].screenPos.x;
					eAp1y = points[edges[edge][1]].screenPos.y;

					eBp0x = points[edges[edge2][0]].screenPos.x;
					eBp0y = points[edges[edge2][0]].screenPos.y;
					eBp1x = points[edges[edge2][1]].screenPos.x;
					eBp1y = points[edges[edge2][1]].screenPos.y;

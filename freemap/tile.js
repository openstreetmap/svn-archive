...
function tile_engine_new(parentname,hints,feedurl,url,lon,lat,zoom,w,h) 
{
	...
	this.latToY = function(lat)
			{ return ((this.lat-modlat(lat))/this.latPerPixel()) + 
							this.theheight/2; }
    //
        ...
        for( var x = this.lon_min; x < this.lon_max ; x+= this.lon_quant ) 
        {
            ...
            for( var y = this.lat_min; y < this.lat_max ; y+= this.lat_quant ) 
            {
                ...
                if(!node) 
                {
                    ...
                    node.src = key + "&zoom=" + this.zoom + "&t=" +
							this.timestamp + this.additional;
					/*
					if(fred)
					{
						alert(node.src);
    ...
    { 
        ...
        this.lat=modlat(lat); 
        this.clean(); 
        this.performzoom(lon,modlat(lat),this.zoom);
        this.drag(0,0); 
    }
...
function normallat(modlat)
{
    var tp = 180 / PI * (2 * Math.atan(Math.exp(modlat * PI / 180)) - PI / 2);
    return tp;
}

function modlat(normallat)
{

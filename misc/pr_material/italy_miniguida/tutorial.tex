...
\usepackage[pdftitle={Introduzione ad OpenStreetMap},pdfauthor={realizzato da Luca Delucchi, Maurizio Napolitano, Alessio Zanol, con il contributo della comunità italiana di OpenStreetMap},colorlinks=true,citecolor=UrlColor,urlcolor=UrlColor,filecolor=UrlColor,linkcolor=UrlColor]{hyperref}
...
Il continuo sviluppo tecnologico ha portato alla costruzione di cellulari sempre più complessi e performanti; molti modelli delle ultime generazioni hanno un ricevitore \gps incorporato, questa situazione ha fatto si che venissero sviluppati programmi per contribuire al progetto e utilizzare in svariati modi i dati di \osm. Esistono software per la maggiorparte dei sistemi operativi (Linux, Android, Symbian, IPhone Windows CE) utilizzati sui cellulari. Nella sezione software vengono presentati alcuni dei più utilizzati.
...
%\section{Hardware}
%DA AGGIUNGERE QUALCOSA SULL'HARDWARE CHE SUPPORTA OSM
...
\soft{PgRouting} \includegraphics{./linux-logo.jpg} \includegraphics{./windows-logo.jpg} \includegraphics{./mac-logo.jpg}: estensione di PostGIS per effettuare calcoli di routing; e-siste osm2pgrouting che importa i file di \osm in PostgreSQL con le tabelle conformi al formato richiesto da PgRouting

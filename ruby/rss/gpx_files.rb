...
begin
  ...
  res = dbh.query('select a.timestamp, a.name, b.count, b.latitude, b.longitude,c.user from (select * from points_meta_table order by timestamp desc limit 20) as a, (select count(*) as count ,gpx_id,latitude, longitude from tempPoints group by gpx_id) as b, (select * from user) as c where a.uid = b.gpx_id and a.user_uid = c.uid order by timestamp desc;')
  ...
  res.each_hash do |row|
    ...
    item.title = row['name']
    item.link = "http://www.openstreetmap.org/edit/applet.jsp?lat=#{row['latitude']}&lon=#{row['longitude']}&scale=10404.917"
    item.description = "GPX file made by #{row['user']} with #{row['count']} points."

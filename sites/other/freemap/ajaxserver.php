...
{
	case "add":
		$q=("insert into freemap_markers (userid,title,description,type,lat,lon,link,private) values ($id,'$_SESSION[gatekeeper]','$cleaned[description]','$cleaned[type]',$cleaned[lat],$cleaned[lon],'$cleaned[link]','$cleaned[private]')");
		mysql_query($q);
		break;

	case "delete":
		$q = 
			"delete from freemap_markers where id=$cleaned[fid] and ".

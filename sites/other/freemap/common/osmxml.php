...
function grabOSM($w0, $s0, $e0, $n0, $datasrc="db", $dw=true,$st=null, 
				$zoom=null, 
				$location="http://www.openstreetmap.org/api/0.3/map")
{
	...
	switch($datasrc)
	{
		...
		case "db": // grab from local database
			grab_direct_from_database($resp2,$w1,$s1,$e1,$n1,$dw,$st);
			break;
		...
		case "osm": // grab from live OSM API
			...
			//echo $resp;
			curl_close($ch);
			$resp2 = parseOSM(explode("\n",$resp),null,null,null,null,$dw,$st);
			break;

		case "file": // grab from local file
			$resp2 = parseOSM($location,$w0,$s0,$e0,$n0,$dw,$st);
			break;
...
function parseOSM($osm,$west=null,$south=null,$east=null,$north=null,
					$dw=true,$st=null)
{
	global $segments, $nodes, $ways, $w, $s, $e, $n, $doWays, $specifiedTags;
...
//				echo "Including segment $curID\n";
		...
		elseif($element=="SEG" && $doWays && $inWay)
		{
			$waySegs[] = $attrs["ID"];
		}
...
function on_end_element($parser, $element)
{
	global $inNode, $nodes, $inSegment, $segments, $curSeg, $curNode, $curID,
			$inWay, $curWay, $ways, $waySegs, $specifiedTags;

	if($element=='NODE')
	{
		$inNode = false;
		if($curNode && keyval_match($curNode['tags'],$specifiedTags))
		{
			$nodes[$curID] = $curNode; // 0.3 UID->ID
		}
	}
	elseif($element=='SEGMENT' && $inSegment)
	{
		$inSegment = false;
		if($curSeg)
		{
	...
	elseif($element=='WAY' && $inWay)
	{
		...
		foreach($waySegs as $segID)
			...
			*/
		if($curWay && keyval_match($curWay['tags'],$specifiedTags))
		{
			$ways[$curID] = $curWay;
		}
	}
...
function keyval_match($curNodeTags, $specifiedTags)
{
	if($specifiedTags)
	{
		foreach($specifiedTags as $keyval)
		{
			foreach($curNodeTags as $k=>$v)
			{
				if($keyval[0]==$k && $keyval[1]==$v)
					return true;
			}
		}
	}

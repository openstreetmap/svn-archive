...
rpm -q xalan-j2 > /dev/null 2>/dev/null
HAS_XALAN_JAR=$?

if [ $HAS_XALAN_JAR == 0 ]; then
	XALAN="java -jar /usr/share/java/xalan-j2.jar"
else
	# No xalan found
	echo "No xalan was found. Please run:"
	echo ""
	echo "   sudo yum install xalan-j2"
	exit 1
fi


# We need a render, either librsvg2-bin or inkscape
rpm -q librsvg2 > /dev/null 2>/dev/null
HAS_RSVG=$?
rpm -q inkscape > /dev/null 2>/dev/null
HAS_INKSCAPE=$?
if [ $HAS_RSVG == 0 ]; then
	...
	echo ""
	echo "   sudo yum install librsvg2"
	echo "   sudo yum install inkscape"
	exit 1
fi
...
# Start rendering
$XALAN -in rules/standard.xml -out data.svg

...
#        end
        ...
        if token != '='
          attrvalue = attrname
#          raise ParseError.new("attribute parse error")
        else
          attrvalue = nextToken
          token = nextToken
        end
...
#        end
    ...
    IgnorableSpaces = /([#{ZenkakuChar.pack('U*')}])[ \x9\n]+([#{ZenkakuChar.pack('U*')}])/u
...
def urls
  ...
  p = YmHTML::Parser.new
  p.eliminateWhiteSpace = true
  $base = nil
  def p.startElement(n, d)
    case n
    when 'link', 'a'
      if d['href']
